...
分割完成后，脚本会对每张照片进行**智能旋转检测**，自动修正倒置或侧转的照片。

### 检测原理

采用多特征融合评分（纯 OpenCV，无需外部 AI 模型）：

| 特征 | 原理 | 权重 |
|------|------|------|
| **人脸位置分析** | 检测多个人脸在上下半部分的分布 | 30% |
| **人脸宽高比分析** | 人脸横向排列暗示 90°/270° 旋转 | 25% |
| **亮度/饱和度梯度** | 天花板 vs 地面的亮度和饱和度差异 | 20% |
| **四角白边分布** | 对角线白边差异检测倒置 | 15% |
| **文字结构方向** | 水平线条集中在顶部暗示倒置 | 10% |
