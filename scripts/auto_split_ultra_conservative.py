#!/usr/bin/env python3
"""
auto_split_ultra_conservative.py — 超保守扫描照片分割工具

功能：
  - 极端保守的裁剪策略，最大程度保留原始内容
  - 更高的检测阈值和更严格的保护机制
  - 仅裁剪确认为白边的区域
"""
import argparse
...
def parse_args():
    ...
    parser.add_argument(
        "-o", "--output",
        default=None,
        help="输出目录（默认：与输入文件同目录下的 ultra_conservative_output/）",
    )
    parser.add_argument(
        "--method",
...
def _detect_colors(img: np.ndarray) -> list:
    """使用颜色聚类查找照片边界"""
    # 转换到 HSV 色彩空间
...
def _detect_variance(img: np.ndarray) -> list:
    """使用方差检测查找照片边界"""
    gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    ...
    # 使用中位数作为阈值
    row_threshold = np.median(row_var) * 1.5  # 更保守
    col_threshold = np.median(col_var) * 1.5
...
def _ultra_conservative_crop(pil_img: Image.Image) -> Image.Image:
    """超保守裁剪 - 仅裁剪确认为白边的区域"""
    ...
    print(f"  [裁剪] 四角白边(254): 左上={tl:.0%}, 右上={tr:.0%}, 左下={bl:.0%}, 右下={br:.0%}")
    print(f"  [裁剪] 对角线差异={diag_diff:.2f}, 平均={avg_corner_white:.0%}")

    # 如果四角平均白边比例极高（>80%），才考虑裁剪
    if avg_corner_white > 0.80:
        print(f"  [裁剪] 检测到极大量白边，进行最小裁剪")

        # 找到纯白区域
        pure_white_rows = np.where(row_white_ratio > 0.9)[0]
        pure_white_cols = np.where(col_white_ratio > 0.9)[0]
        ...
        if len(pure_white_rows) > 0 and len(pure_white_cols) > 0:
            ...
            # 确保裁剪后仍然保持足够的照片内容
            MIN_CONTENT = 0.95  # 至少保留95%的内容
            if (y_end - y_start < h * MIN_CONTENT) or (x_end - x_start < w * MIN_CONTENT):
                print(f"  [保护] 裁剪会损失过多内容，不进行裁剪")
                return pil_img

            # 执行最小裁剪
            cropped = arr[y_start:y_end + 1, x_start:x_end + 1]
            result = Image.fromarray(cropped)
            print(f"  [OK] 仅裁剪纯白区域，保留 {((y_end - y_start + 1) / h * 100):.1f}% 内容")
            return result

    print(f"  [保护] 未检测到需要裁剪的纯白区域，保留完整照片")
    return pil_img
...
def save_images(boxes: list, img: np.ndarray, output_dir: Path, suffix: str, start_index: int = 1,
    ...
    for i, (x, y, w, h) in enumerate(valid_boxes, start_index):
        ...
        # 超保守裁剪
        pil_img = _ultra_conservative_crop(pil_img)
...
def main():
    ...
    if args.output:
        ...
        if input_path.is_file():
            output_dir = input_path.parent / "ultra_conservative_output"
        else:
            output_dir = input_path / "ultra_conservative_output"
